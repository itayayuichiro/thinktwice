<?php 
    ...
    if (!empty($_POST['login_id']) && !empty($_POST['login_pass'])) {
    	$sql = "select * from users where id = '{$_POST['login_id']}' and password = '{$_POST['login_pass']}'";
...
<h1>簡易掲示板</h1>
     ...
     if (!empty($login)) {
	    ...
	    入力フォーム<br>
	    <?php if (!$edit_flag): ?>
	    <form action="./mission_3-7.php" method="post" accept-charset="utf-8">
	        名前：<input type="text" name="name" value="<?php echo $_COOKIE['name'];  ?>" placeholder=""><br>
