<?php 
  ...
  if (!empty($_POST['name']) && !empty($_POST['comment']) && empty($_POST['edit_num']) ) {
    ...
    for ($i=0; $i < count($array); $i++) { 
    	$content =  explode("<>",$array[$i]);
    	if ($content[0] == $_POST['edit_num']) {
    		$new_text = $new_text . $content[0]."<>" .$_POST['name']."<>".$_POST['comment']."<>".$content[3]."<>". PHP_EOL;
    	}else{
...
<body>
	入力フォーム<br>
	<?php if (!$edit_flag): ?>
	<form action="./mission_2-5.php" method="post" accept-charset="utf-8">
	...
	<?php 
        ...
        for ($i=0; $i < count($array); $i++) { 
        	$content =  explode("<>",$array[$i]);
        	echo $content[0]." 名前 : ".$content[1]." ".$content[3]."<br>";
        	echo $content[2]."<br>";
        	echo "<hr>";
        }
